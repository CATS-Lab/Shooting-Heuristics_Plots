...
text(ts_shadow(1)-.2,ps_shadow(1)+4,'$t''^-$');
...
mid_scale=0;
t_mid = t_now + mid_scale*(t_m-t_now);
p_mid = p_now+ v_now*(t_mid-t_now)+0.5*a_next*(t_mid-t_now)^2;
text(t_mid, p_mid+7,'$(l,v,a^+,t^-,t^\texttt{m})$','Rotation',50);
...
text(ts_dec(end)+.2, ps_dec(end)-2,'$t^+$');
mid_scale=0;
t_mid = t_m + mid_scale*(t_tangent-t_m);
p_mid = p_m+ v_m*(t_mid-t_m)+0.5*a_comfort_dec*(t_mid-t_m)^2;
text(t_mid, p_mid-7,'$(l^\texttt{m},v^\texttt{m},a^-,t^\texttt{m},t^+)$','Rotation',45);
